...
describe("streamCommandCode — Retry-After header", () => {
  ...
  it("refuses Retry-After beyond the default 60s ceiling", async () => {
    // No maxRetryDelayMs option: the effective cap is DEFAULT_MAX_RETRY_DELAY_MS
    // (60000 in policy.ts). A 300s Retry-After is far beyond it, so the request
    // must fail fast instead of sleeping for minutes — one attempt, no delay.
    server.mockResponseQueue([
      {
        type: "error",
        status: 429,
    ...
    assert.deepEqual(eventTypes(events), ["start", "error"])
    const lastCeiling = events.at(-1)
    if (lastCeiling?.type !== "error") throw new Error("expected error")
    assert.match(lastCeiling.error.errorMessage ?? "", /exceeds max 60000ms/)
  })

  it("honors Retry-After up to the default 60s ceiling", async () => {
    // 59s sits inside the default cap, so the wait must be honored and the
    // request retried (locks the ceiling boundary from below).
    server.mockResponseQueue([
      {
        type: "error",
        status: 429,
    ...
    assert.deepEqual(delays, [59_000])
    assert.equal(server.requestCount(), 2)
    assert.equal(events.at(-1)?.type, "done")
  })
})
