...
describe("streamCommandCode — host headers", () => {
  it("suppresses default headers set to null (case-insensitively) and applies overrides", async () => {
    ...
    const stream = streamCommandCode(makeModel(), makeContext(), {
      apiKey: "user_test",
      headers: {
        "x-cli-environment": null, // exact-case suppression
        "X-Taste-Learning": null, // cross-case suppression of "x-taste-learning"
        "x-custom-header": "yes",
      },
    })
    await collectEvents(stream)

    assert.ok(captured, "transport must call fetch")
    assert.ok(!("x-cli-environment" in captured), "exact-case null must suppress the default")
    assert.ok(
      !("x-taste-learning" in captured),
      "a null header value must suppress the matching default regardless of case",
    )
    assert.equal(captured["x-custom-header"], "yes")
    assert.equal(captured["x-co-flag"], "false")
  })
})
