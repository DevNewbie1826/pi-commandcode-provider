  ...
  const server = createServer((req, res) => {
    ...
    req.on("end", () => {
      try {
        ...
        if (parsed.error) {
          // Error callbacks must carry the same state binding as success callbacks;
          // otherwise any local process could terminate an active login.
          const errorState = typeof parsed.state === "string" ? parsed.state : ""
          if (expectedState !== undefined && errorState !== expectedState) {
            res.writeHead(403)
        ...
        // Success callbacks must carry the same state binding as error callbacks;
        // otherwise any local process could inject credentials into an active login.
