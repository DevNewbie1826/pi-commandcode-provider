...
import { parseAccountFile } from "./accounts/schema.ts"
...
export function getApiKey(
  ...
  for (const authPath of authPaths) {
    try {
      ...
      if (parsed.version === 1 || Object.hasOwn(parsed, "accounts")) {
        const accountFile = parseAccountFile(parsed)
        const now = Date.now()
        const account = accountFile.accounts.find(
          (candidate) =>
            candidate.enabled && (candidate.retryAt === undefined || candidate.retryAt <= now),
        )
...
export { toJsonSchema, toolsToJson } from "./converters/schema.ts"
